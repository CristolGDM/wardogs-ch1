 ...
 * need to include frames(x) where x is the number of frames per character in it.
 *
 * For example, a characterset with 8 frames per character could be named like:
 * MainHero_frames(8).png
 *
...
Window_Base.prototype.drawCharacter = function(characterName, characterIndex, x, y) {
	var setFrame = characterName.match(/frames\((.*)\)/i);
...
Sprite_Character.prototype.setCharacterBitmap = function() {
	var setFrame = this._characterName.match(/frames\((.*)\)/i);
	if (setFrame) {
		this._cframes = Number(setFrame[1]);
